        ...
        let li = `
            <li class="task list-group-item">
                ...
                <div class="dropdown">
                    ...
                    <li><a onclick='editTask(${gorev.id}, "${gorev.gorevAdi}")' class="dropdown-item" href="#"><i class="fa-solid fa-pen"></i> Update</a></li>
                    </ul>
                </div>
            </li>
        `
...
function newTask() {
    ...
    if (textInput.value == "") {
        ...
        if(!isEditTask) {
            //ekleme
            gorevListesi.push({"id": gorevListesi.length + 1, "gorevAdi": textInput.value })
        } else {
            //güncelleme
            for (let gorev of gorevListesi) {
                if (gorev.id == editId) {
